import resume from "../Resume_PaoloPedrigal.pdf";
import selfie from "./selfie-2023.jpg";
import "./About.css";

interface AboutMe {
  aboutMeHeading: string;
  aboutMeDescription: string;
  aboutMeObjective: string;
}

const aboutMeInfo: AboutMe = require("./about.json");

export default function About() {
  return (
    <div className="about-me-container">
      <div className="about-me-pic-container">
        <img
          src={selfie}
          alt="Picture of Paolo Pedrigal"
          className="about-me-pic"
        />
      </div>
      <div>
        <h2>{aboutMeInfo.aboutMeHeading}</h2>
        <p>{aboutMeInfo.aboutMeDescription}</p>
        <i>
          <span>{aboutMeInfo.aboutMeObjective}&nbsp;</span>
          <span>
            Check out my&nbsp;
            <a href={resume} target="_blank">
              resume
            </a>
            &nbsp;or&nbsp;
            <a href="mailto:paolopedrigal@gmail.com">contact me.</a>
          </span>
        </i>
      </div>
      <div className="custom-shape-divider-bottom-1696837199">
        <svg
          data-name="Layer 1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 1200 120"
          preserveAspectRatio="none"
        >
          <path
            d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z"
            className="shape-fill"
          ></path>
        </svg>
      </div>
    </div>
  );
}
